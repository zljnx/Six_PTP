...
public interface LxgCorporateDao {
    ...
    LxgInstitutionCombination queryCorporateView(@Param("lxgInstitutionCombination") LxgInstitutionCombination lxgInstitutionCombination);
     //登录
    @Select("select count(*) from p2p_userinfo where loginname=#{loginname} and userpwd=#{userpwd}")
    ...
    //注册
    @Insert("insert into p2p_userinfo(loginname,userpwd,userphone) values (#{userInfo.loginname},#{userInfo.userpwd},#{userInfo.userphone})")
